...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($mode, ['predict', 'predict_manual'], true)) {
    ...
    try {
        ...
        /*
         * Manual ส่งเฉพาะค่าที่เปลี่ยนตามเวลา ส่วน depth ดึงจาก station อัตโนมัติ
         */
        if ($mode === 'predict_manual') {
            $manualValues = [$sst, $chl_a, $sss, $rainfall, $wind_speed, $air_temperature, $wind_direction];
            ...
            foreach ([['--sst',$sst],['--chl_a',$chl_a],['--sss',$sss],['--rainfall',$rainfall],
                      ['--wind_speed',$wind_speed],['--air_temperature',$air_temperature],
                      ['--wind_direction',$wind_direction]] as $pair) {
                $args[] = $pair[0];
                $args[] = (float)$pair[1];
            }
        }
...
function featureLabel($feature)
{
    $labels = [
        ...
        'chlorophyll_a' => 'Chlorophyll-a', 'sss' => 'Sea Surface Salinity (SSS)',
        'rainfall' => 'Rainfall', 'wind_speed' => 'Wind Speed',
        'air_temperature' => 'Air Temperature',
        'wind_sin' => 'Wind Direction (sin)', 'wind_cos' => 'Wind Direction (cos)',
        'depth' => 'Depth (from Station)'
    ];
...
<div class="container page-container">
    ...
    <div class="card shadow-sm border-0 mb-4 classification-card">
        ...
        <div class="card-body">
            ...
            <p class="info-note mb-4">
                ...
                กรอกค่าทางทะเลและสภาพอากาศสำหรับเดือน ปี และจังหวัดที่ต้องการ
                โดย Depth ไม่ต้องกรอกเอง เพราะระบบจะดึงจาก Station ที่เลือกโดยอัตโนมัติ
            ...
            <form method="post">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" name="sss" class="form-control" step="0.001" min="0" max="50" value="<?= htmlspecialchars($sss) ?>" placeholder="เช่น 25.48" required>
                    </div>

                    <div class="col-md-3 mb-3">
                        <label class="form-label">Air Temperature (°C)</label>
                        <input type="number" name="air_temperature" class="form-control" step="0.01" min="-10" max="60" value="<?= htmlspecialchars($air_temperature) ?>" placeholder="เช่น 26.90" required>
                    </div>

                    <div class="col-md-3 mb-3">
                        <label class="form-label">Wind Direction (°)</label>
                        <input type="number" name="wind_direction" class="form-control" step="0.01" min="0" max="360" value="<?= htmlspecialchars($wind_direction) ?>" placeholder="เช่น 100.00" required>
                    </div>
    ...
    <div class="card shadow-sm border-0 mb-4 classification-card">
        ...
        <div class="card-body">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6 mb-3">
                    <div class="metric-box">
                        <div class="metric-title">Sea Surface Salinity (SSS)</div>
                        <div class="environment-value">
                            <?= isset($input['sss']) ? number_format((float)$input['sss'], 3) . ' PSU' : '-' ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="metric-box">
                        <div class="metric-title">Depth (from Station)</div>
                        <div class="environment-value">
                            <?= isset($input['depth']) ? number_format((float)$input['depth'], 2) . ' m' : '-' ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6 mb-3">
                    <div class="metric-box">
                        ...
                        <div class="environment-value">
                            <?= isset($input['air_temperature']) ? number_format((float)$input['air_temperature'], 2) . ' °C' : '-' ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="metric-box">
                        ...
                        <div class="environment-value">
                            <?= isset($input['wind_direction']) ? number_format((float)$input['wind_direction'], 2) . '°' : '-' ?>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-3 mb-3">
                    <div class="metric-box">
                        ...
                        <div class="metric-value"><?= percent($metrics['balanced_accuracy'] ?? 0) ?></div>
                    </div>
                </div>
    ...
    <div class="card shadow-sm border-0 mb-4">
        ...
        <div class="card-body">
            ...
            <p class="info-note">
                โมเดล Random Forest Classifier สำหรับจำแนกระดับปริมาณปลาทูจากข้อมูลการขึ้นท่า
                ร่วมกับข้อมูลตำแหน่ง เวลา สภาพแวดล้อมทางทะเล และสภาพอากาศ
            </p>
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <ul class="mb-0">
                        ...
                        <li>ข้อมูลทางทะเล: SST, Chlorophyll-a และ SSS</li>
                        <li>ข้อมูลอากาศ: Rainfall, Wind Speed และ Air Temperature</li>
                        <li>Wind Direction แปลงเป็น sin และ cos เพื่อรองรับข้อมูลทิศทางแบบวงกลม</li>
                        <li>Depth ดึงจากตาราง Station อัตโนมัติตามจังหวัดที่เลือก</li>
                    </ul>
                </div>
            </div>
            ...
            <div class="alert alert-light border mt-3 mb-0">
                ...
                ให้ผู้ใช้กรอกค่าที่เปลี่ยนตามเวลาเอง แต่ Station ID และ Depth ยังคงดึงจาก Station เพื่อให้ตรงกับตำแหน่งอ้างอิง
            </div>
